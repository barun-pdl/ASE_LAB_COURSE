...
for (i=0; i<16; i++) {   
   ...
   /* MB Code */
    CAN_0.BUF[i].CS.B.CODE = 0xC ; // (1100) Transmit data frame unconditionally once. After transmission, the MB
								   //automatically returns to the INACTIVE state.
    /* Standard format */
 	CAN_0.BUF[i].CS.B.IDE = 0 ;	 // Use standard length ID
    /* SRR */
    CAN_0.BUF[i].CS.B.SRR = 0; // Valid only in extended format (Can be 1?)
    /* Data Frame */
    CAN_0.BUF[i].CS.B.RTR = 0 ; // Data frame is to be transmitted (Not a remote frame)
    /* Data Length */
    CAN_0.BUF[i].CS.B.LENGTH = 0x2 ; // This 4-bit field is the length  2 data bytes 
    /* STD_ID */	
    CAN_0.BUF[i].ID.B.STD_ID = 420 ; // *****CHECK IP ADDRESS OF COMPUTER USING IPCONFIG AND USE THE LAST LAST 3 DIGITS
}
/********************************************************************
