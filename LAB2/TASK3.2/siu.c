...
void SIU_Init(void) {
  ...
  // TO CONFIGURE THE PINS FOR RECEPTION AND TRANSMISSION
  //PCR For Tx = 16 and for Rx= 17 ==>1 is to be written in PA of PCR
  
	
    SIU.PCR[16].B.PA = 1;	// FLEXCAN_0 TxD connected at PB0 
    SIU.PCR[16].B.IBE = 0 ; // Disable input buffer
    SIU.PCR[16].B.OBE = 1;	// Enable Output buffer
    
    SIU.PCR[17].B.PA = 1;	  // FLEXCAN_0 RxD connected at PB1
    SIU.PCR[17].B.IBE = 1;   // Enable Input buffer
    SIU.PCR[17].B.OBE = 0;   // Disable Output buffer
